                ...
                <v-flex pl-5 xs6>
                  ...
                  >
                    <template v-slot:activator="{ on }">
                      <v-text-field :value="computedDateFormatted" clearable readonly v-on="on"></v-text-field>
                    </template>
                    <v-date-picker outline v-model="dob" @change="date_menu = false"></v-date-picker>
                  </v-menu>
                </v-flex>
              </v-layout>
  ...
  export default {
    ...
    methods: {
      ...
      */
        ...
        this.$http
          .patch("/user/update/" + this.$store.getters.getUserKey, updateData)
          .catch(function(err) {
            console.log(err);
          })
          .then(function(res) {
